    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-start">
          ...
          <Button
            type="button"
            disabled={!isValidE164(phoneInput)}
            onClick={async () => {
        ...
        {debugCode ? (
          <p className="mt-2 text-xs text-[var(--signal-deep)]">
            Dev code: {debugCode}
          </p>
        ) : null}
      </section>
      ...
      <section>
          ...
          <div className="mt-4 space-y-3">
            ...
            {qrDataUrl ? (
              ...
              <img
                src={qrDataUrl}
                alt="2FA QR code"
                className="rounded-2xl border-2 border-[var(--mist-strong)] bg-[var(--surface)] p-2"
                width={200}
                height={200}
              />
            ) : null}
        ...
        {recoveryCodes.length ? (
          <ul className="mt-4 grid gap-1 rounded-2xl border-2 border-[var(--mist-strong)] p-4 font-mono text-xs">
            {recoveryCodes.map((c) => (
              <li key={c}>{c}</li>
            ))}
          </ul>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setE2eReady(true);
              setMsg("Encryption keys ready on this device.");
            } catch {
              setMsg("Could not set up encryption keys in this browser.");
            } finally {
              setE2eBusy(false);
            }
          }}
        >
          {e2eReady ? "Refresh encryption keys" : "Set up encryption keys"}
        </Button>
      </section>
