...
export default function AdminVerificationPage() {
  const { data, loading, error, reload } = useAdminJson<{
    requests: Array<{
      ...
      notes: string | null;
      createdAt: string;
      user: { handle: string | null; displayName: string | null; isVerified: boolean };
    }>;
  }>("/api/admin/verification");

  async function review(requestId: string, action: "approve" | "reject" | "needs_info") {
    const adminNote =
      action === "needs_info"
        ? window.prompt("What additional information is needed?") || undefined
        : undefined;
    await adminPost("/api/admin/verification", { requestId, action, adminNote });
    await reload();
  }
