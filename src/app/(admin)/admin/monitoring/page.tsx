...
export default function AdminMonitoringPage() {
  const { data, loading, error } = useAdminJson<{
    ...
    database: { users: number; posts: number; messages: number; storageBytes: number };
    server: { uptimeSec: number; memoryMb: number; node: string };
    securityLogs: Array<{ id: string; type: string; severity: string; createdAt: string }>;
      ...
      {data ? (
        <>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <Panel>
              <h2 className="font-[family-name:var(--font-syne)] text-lg font-semibold">Database</h2>
            ...
            <Panel>
              <h2 className="font-[family-name:var(--font-syne)] text-lg font-semibold">Security logs</h2>
              <ul className="mt-3 max-h-72 space-y-2 overflow-auto text-xs text-[var(--muted)]">
                {data.securityLogs.map((l) => (
                  <li key={l.id}>
                    [{l.severity}] {l.type} · {new Date(l.createdAt).toLocaleString()}
                  </li>
                ))}
              </ul>
            </Panel>
