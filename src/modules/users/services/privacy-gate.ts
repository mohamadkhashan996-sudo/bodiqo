...
async function audienceAllows(
  ...
  const [followsTarget, targetFollows] = await Promise.all([
    ...
    prisma.follow.findUnique({
      where: {
        followerId_followingId: { followerId: targetId, followingId: actorId },
      },
      select: { id: true },
    }),
  ]);
...
async function canContact(
  actorId: string,
  targetId: string,
  setting: "whoCanMessage" | "whoCanCall",
) {
  ...
  return audienceAllows(
    ...
    targetId,
    target?.[setting] ??
      (setting === "whoCanMessage" ? "EVERYONE" : "FOLLOWERS"),
  );
}
...
/** Allow opening a message request when direct messaging is restricted (not NOBODY/block). */
export async function canRequestMessage(actorId: string, targetId: string) {
  if (actorId === targetId) return false;
  const [privacy, blocked, restricted, target] = await Promise.all([
    prisma.privacySettings.findUnique({
      where: { userId: targetId },
  ...
  if (blocked || restricted || !target || target.status !== "ACTIVE") {
    return false;
  }
  const audience = privacy?.whoCanMessage ?? "EVERYONE";
  if (audience === "NOBODY") return false;
  // Already allowed to message → not a "request" path.
  if (await audienceAllows(actorId, targetId, audience)) return false;
  return true;
}
...
export async function canFollow(actorId: string, targetId: string) {
  if (actorId === targetId) return false;
  const [privacy, blocked, target] = await Promise.all([
    prisma.privacySettings.findUnique({
      where: { userId: targetId },
  ...
  if (blocked || !target || target.status !== "ACTIVE") return false;

  const audience = privacy?.whoCanFollow ?? "EVERYONE";
  if (audience === "NOBODY") return false;

  // Private accounts: anyone may send a follow request (approval is the gate).
  // Restrictive audiences only apply to public instant follows.
  if (target.isPrivate) return true;

  return audienceAllows(actorId, targetId, audience);
}
...
export function canSeeStories(viewerId: string | undefined, authorId: string) {
  if (!viewerId) {
    return prisma.privacySettings
      ...
      .then(
        (privacy) => (privacy?.whoCanSeeStories ?? "EVERYONE") === "EVERYONE",
      );
  }
...
export async function filterVisibleStoryAuthors(
  ...
  if (!viewerId) {
    ...
    return new Set(
      unique.filter((id) => (byUser.get(id) ?? "EVERYONE") === "EVERYONE"),
    );
  }
...
export async function canSeeOnlineStatus(viewerId: string, targetId: string) {
  ...
  return audienceAllows(
    viewerId,
    targetId,
    privacy?.whoCanSeeOnline ?? "FOLLOWERS",
  );
}
...
export async function filterOnlineStatusViewers(
  ...
  const unique = [...new Set(candidateViewerIds)].filter(
    (id) => id !== targetId,
  );
...
export async function canMention(actorId: string, targetId: string) {
  ...
  return audienceAllows(
    actorId,
    targetId,
    privacy?.whoCanMention ?? "EVERYONE",
  );
}
